 ...
 *
 * Run with: node --test src/lib/vertex-ai/openai.test.ts
 * Or: npm test
 *
 * Prerequisites:
 ...
 * - OpenAI models enabled in your Vertex AI region
 *
 * Note: These tests make real API calls to Vertex AI and will incur costs.
 *       OpenAI models may not be available in all Vertex AI regions.
 */
...
describe('OpenAI GPT Provider', () => {
  ...
  it('should throw error for invalid model name', async () => {
    console.log('\n🧪 Testing OpenAI GPT error handling (invalid model)');

    await assert.rejects(
      async () => {
        await generateContent(
          'invalid-gpt-model-xyz',
          'test',
          { temperature: 0 }
        );
      },
