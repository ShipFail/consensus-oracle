/**
 * Meta Llama Provider Tests
 *
 * Run with: node --test src/lib/vertex-ai/meta.test.ts
 * Or: npm test
 *
 * Prerequisites:
 ...
 * - Llama models enabled in your Vertex AI region
 *
 * Note: These tests make real API calls to Vertex AI and will incur costs.
 *       Llama models may not be available in all Vertex AI regions.
 */

import { describe, it } from 'node:test';
import assert from 'node:assert';
import { generateContent, MODELS } from './meta';

describe('Meta Llama Provider', () => {
  it('should generate content with deterministic settings ("Tell me a joke")', async (t) => {
    ...
    console.log(`\n🧪 Testing Meta Llama: ${MODELS.LLAMA_4_MAVERICK_17B_128E_INSTRUCT}`);
    ...
    try {
      ...
      if (message.includes('404')) {
        t.skip(
          `Meta Llama model unavailable in region ${process.env.GCP_LOCATION || 'us-central1'}: ${message}`
        );
        return;
      }
      throw error;
  ...
  it('should throw error for invalid model name', async () => {
    console.log('\n🧪 Testing Meta Llama error handling (invalid model)');

    await assert.rejects(
      async () => {
        await generateContent(
          'invalid-llama-model-xyz',
          'test',
          { temperature: 0 }
        );
      },
